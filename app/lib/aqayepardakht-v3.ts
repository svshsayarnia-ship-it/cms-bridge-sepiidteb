...
const GATEWAY_API_BASE = "https://api.aqayepardakht.ir/v3";
const GATEWAY_START_BASE = "https://api.aqayepardakht.ir/startpay/";
...
function readTransidHistory(order: WooOrder) {
  const values = metaValues(order, GATEWAY_TRANSIDS_META_KEY);
  const result = new Set<string>();
  for (const value of values) {
    if (Array.isArray(value)) {
      value.forEach((entry) => {
        if (typeof entry === "string" && entry) result.add(entry);
      });
...
function gatewayErrorMessage(code: string) {
  switch (code) {
    case "-1": return "مبلغ پرداخت برای درگاه معتبر نیست.";
    case "-2": return "پین درگاه تنظیم نشده است.";
    case "-3": return "آدرس بازگشت درگاه معتبر نیست.";
    case "-4": return "مبلغ واردشده برای درگاه نامعتبر است.";
    case "-5": return "مبلغ خارج از بازه مجاز درگاه است.";
    case "-6": return "پین درگاه معتبر نیست.";
    case "-11": return "درگاه هنوز تأیید نشده یا غیرفعال است.";
    case "-12": return "ارسال درخواست برای این پذیرنده مجاز نیست.";
    default: return "درگاه پرداخت درخواست را نپذیرفت.";
  }
}
...
async function gatewayRequest<T extends GatewayCreateResponse | GatewayVerifyResponse>(
  path: "pay" | "verify",
...
export async function createAqayePardakhtPayment(input: {
  orderId: number;
  ...
  const { pin, callback } = gatewayConfig();
  const amount = amountInToman(order);
  const callbackUrl = new URL(callback);
  callbackUrl.searchParams.set("order_id", String(order.id));

  const response = await gatewayRequest<GatewayCreateResponse>("pay", {
    pin,
    amount,
  ...
  const trackingCode = String(response.tracking_code ?? "").trim();
  if (response.status !== "success" || !trackingCode) {
    throw new AqayePardakhtError(
      "ایجاد تراکنش در درگاه انجام نشد. دوباره تلاش کنید.",
      502,
      `gateway_create_${String(response.code ?? "failed")}`,
    );
  }

  await rememberTransid(order, trackingCode);

  return {
    transid: trackingCode,
    url: `${GATEWAY_START_BASE}${encodeURIComponent(trackingCode)}`,
    amount,
  };
}
...
export async function verifyAqayePardakhtCallback(input: {
  ...
  const history = readTransidHistory(order);
  if (!history.includes(transid)) {
    throw new AqayePardakhtError(
  ...
  const callbackStatus = String(input.status ?? "").trim().toLowerCase();
  if (callbackStatus && !["1", "success", "ok"].includes(callbackStatus)) {
    return {
      ok: false as const,
      orderId: order.id,
  ...
  const amount = amountInToman(order);
  const response = await gatewayRequest<GatewayVerifyResponse>("verify", {
    pin,
    tracking_code: transid,
    amount,
  });
