...
async function gatewayRequest<T extends GatewayCreateResponse | GatewayVerifyResponse>(
  ...
  try {
    const response = await fetch(`https://panel.aqayepardakht.ir/api/v2/${path}`, {
      method: "POST",
      headers: {
...
export async function createAqayePardakhtPayment(input: {
  ...
  const response = await gatewayRequest<GatewayCreateResponse>("create", {
    pin,
    amount,
    callback,
    callback_method: "GET",
  ...
  if (response.status !== "success" || typeof response.transid !== "string" || !response.transid.trim()) {
    throw new AqayePardakhtError(
  ...
  const transid = response.transid.trim();
  ...
  return {
    transid,
    url: `${base}${encodeURIComponent(transid)}`,
    amount,
  };
}
